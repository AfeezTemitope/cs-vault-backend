...
const sendWelcomeEmail = async ({ email, fullName, matricNumber, password, role, isReset = false }) => {
  const roleLabel = role === 'lecturer' ? 'Lecturer' : role === 'admin' ? 'Admin' : 'Student';
  const subject = isReset
    ? 'CS-Vault — Your Password Has Been Reset'
    : 'Welcome to CS-Vault — Your Login Details';

  const heading = isReset ? 'Password Reset' : 'Welcome to CS-Vault';
  const message = isReset
    ? 'Your password has been reset. Here are your new login details:'
    : `Your <strong>${roleLabel}</strong> account has been created on CS-Vault. Here are your login details:`;
    ...
    await resend.emails.send({
      ...
      html: `
        <div style="font-family: sans-serif; max-width: 480px; margin: auto; padding: 32px; background: #0f0f0f; color: #f0f0f0; border-radius: 12px;">
          ...
          <p style="color: #f87171; font-size: 13px;">⚠️ Please change your password after logging in.</p>
          ...
          <p style="margin-top: 32px; font-size: 12px; color: #555;">If you did not request this, please contact your lecturer or admin.</p>
        </div>
      `,
    });
